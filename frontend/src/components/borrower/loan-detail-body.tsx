...
/**
 * One loan's full economics — what was sent, the upfront deductions, the cost (interest + late
 * penalty), what's been paid (with dates + proofs) and what's still owed. Its own file rather than
 * an export of `loan-details-dialog` because it is no longer dialog-specific: `/loans` renders it
 * inline on a past-loan card and the dialog renders the very same component, so the two can never
 * drift apart. Self-contained: it fetches the loan, its prepayment/penalty-aware outstanding, and
 * its payments, all keyed so an inline render and the dialog share one cache entry.
 */
export function LoanDetailBody({
  ...
  loanId: number | null;
  /** Off for a collapsed/closed host, so N unseen cards don't each fire three requests. */
  enabled?: boolean;
  /** Fired when the repay CTA is followed — lets a dialog host close itself first. */
  onNavigate?: () => void;
}) {
  const active = enabled && loanId != null;
  ...
  if (loanQuery.isLoading) return <div className="h-48 animate-pulse rounded bg-grey-100" />;
  ...
  return (
    <div className="space-y-5">
      ...
      {(loan.status === "ACTIVE" || loan.status === "OVERDUE") && (
        <Link href="/repay" className="btn btn-gold btn-block" onClick={onNavigate}>
