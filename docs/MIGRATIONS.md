...
| `V68__lead_import_job.sql` | `lead_import_job` (one durable row per uploaded lead file: status, counts, first-50 `issues_json`) + the `lead.pan` index the dedup needs — the CSV import moved browser → S3 → streamed chunks, so the work now outlives the request that started it |
| `V69__customer_limit_override.sql` | `customer_limit_override` — sparse ADMIN per-**customer** eligible-limit override that wins over the 25%-of-salary formula in `EligibilityService.effectiveLimitPaise`; editing `loan_application.eligible_limit` could not work, since it is re-derived on payslip verification, salary edits and every reborrow |
| `V70__lead_outcome_and_dsa_note.sql` | `lead.{lead_outcome, dsa_note}` — the outreach outcome (NEW/OUTREACHED/REJECTED; `CONFIRMED` deliberately **not** in the CHECK, it is resolved on read from the PAN-attributed application) on its own axis from the seven `call_status` dispositions, plus the one note the uploading DSA may read (never `remarks`, which stays internal) |
| `V71__perf_insurance_indexes.sql` | four forward-looking indexes — `application_event(action, at)` (dashboard trends), `loan(status, due_date)` (collections worklist), `payment(status, paid_on)` (trends + ledger window), partial `loan_application(loan_id) where not null` (`findByLoanIdIn`). No measurable effect at today's row counts; plain `CREATE INDEX` because Flyway wraps each migration in a transaction, so `CONCURRENTLY` is unavailable. See `docs/perf/UI_PERFORMANCE_INVESTIGATION_2026-09-16.md` §2.7 |
